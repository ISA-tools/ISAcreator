...
/**
 * MappingConfidenceListCellRenderer
 *
...
public class MappingConfidenceListCellRenderer extends JComponent
    ...
    public MappingConfidenceListCellRenderer() {
        ...
        for (Component c : components) {
            ...
            if (c instanceof ConfidenceLevelCellImage) {
                if (value instanceof FieldMapping) {
                    ((ConfidenceLevelCellImage) c).setConfidenceLevel((FieldMapping) value);
                }
            } else {
                if (selected) {
    ...
    class ConfidenceLevelCellImage extends JPanel {
        ...
        public void setConfidenceLevel(FieldMapping fieldMapping) {

            ConfidenceLevel level = fieldMapping.getConfidenceLevel();

            if (level == ConfidenceLevel.ONE_HUNDRED_PERCENT) {
                itemSelectedIndicator.setIcon(oneHundredPercent);
            } else if (level == ConfidenceLevel.SEVENTY_FIVE_PERCENT) {
                itemSelectedIndicator.setIcon(seventyFivePercent);
            } else if (level == ConfidenceLevel.FIFTY_PERCENT) {
                itemSelectedIndicator.setIcon(fiftyPercent);
            } else if (level == ConfidenceLevel.TWENTY_FIVE_PERCENT) {
                itemSelectedIndicator.setIcon(twentyFivePercent);
            } else {
