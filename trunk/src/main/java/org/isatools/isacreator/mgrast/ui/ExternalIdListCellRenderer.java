...
public class ExternalIdListCellRenderer extends JComponent
        implements ListCellRenderer {

    @InjectedResource
    private ImageIcon idEntered, noIdEntered;

    private DefaultListCellRenderer listCellRenderer;

    public ExternalIdListCellRenderer() {
    ...
    public Component getListCellRendererComponent(JList list, Object value, int index, boolean selected, boolean cellGotFocus) {
        ...
        SampleExternalIds sampleObj = (SampleExternalIds) value;

        for (Component c : components) {
            ((JComponent) c).setToolTipText(value.toString());
            if (c instanceof IDEnteredListCellmage) {
                ((IDEnteredListCellmage) c).checkIsIdEntered(sampleObj);
            } else {
                if (selected) {
    ...
    class IDEnteredListCellmage extends JPanel {
        ...
        IDEnteredListCellmage() {
            setLayout(new BoxLayout(this, BoxLayout.LINE_AXIS));
            itemSelectedIndicator = new JLabel(noIdEntered);

            add(itemSelectedIndicator);
            add(Box.createHorizontalStrut(2));
        }

        public void checkIsIdEntered(SampleExternalIds sampleId) {

            boolean valueEntered = false;
            for (ExternalResources er : sampleId.getExternalIds().keySet()) {
                if (!sampleId.getExternalIds().get(er).trim().equals("")) {
                    valueEntered = true;
                    break;
                }
            }
            ...
            if (valueEntered) {
                itemSelectedIndicator.setIcon(idEntered);
            } else {
                itemSelectedIndicator.setIcon(noIdEntered);
            }
        }
    }
}
