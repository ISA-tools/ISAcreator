...
public class CustomTreeRenderer implements TreeCellRenderer {
    ...
    @InjectedResource
    private ImageIcon node, nodeOpen, nodeClosed;


    public CustomTreeRenderer(boolean preferredSize, int maxTextWidth, int maxTextHeight) {
        ...
        icon = new JLabel();
        text = UIHelper.createLabel("", UIHelper.VER_12_BOLD, UIHelper.DARK_GREEN_COLOR);

        Dimension size = new Dimension(maxTextWidth, maxTextHeight);
        if (preferredSize) {
    ...
    public Component getTreeCellRendererComponent(JTree tree, Object val, boolean selected, boolean expanded, boolean leaf, int index, boolean hasFocus) {
        ...
        if (leaf) {
            ...
            icon.setIcon(nodeOpen);
        } else {
            icon.setIcon(nodeClosed);
        }
